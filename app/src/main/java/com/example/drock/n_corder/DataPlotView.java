...
public class DataPlotView extends View {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        mExampleColor = a.getColor(
                R.styleable.DataPlotView_exampleColor,
                mExampleColor);
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mData != null) {
            ...
            List<Measurement> dataSubset;
            synchronized (mData) {
                dataSubset = mData.subList(Math.max(0, mData.size() - contentWidth), mData.size());
            }
            for (Measurement m : dataSubset) {
                dataMin = Math.min(dataMin, m.getValue());
                dataMax = Math.max(dataMax, m.getValue());
            }
            float yRange = dataMax - dataMin;

            //Plot Values
            if (dataSubset.size() > 0) {
                float x = paddingLeft;
                if (yRange <= Float.MIN_NORMAL) {
                    yRange = 1.f;
                    dataMin -= yRange / 2.0f;
                }
                for (Measurement m : dataSubset) {
                    float v = m.getValue();
                    float y = contentHeight * ((yRange - (v - dataMin)) / yRange) + paddingTop;
